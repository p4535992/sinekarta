 ...
 * @author adeprato
 */
public class PDFSignatureServiceItext 
		extends AbstractSignatureService < SignCategory,
										   SignDisposition.PDF,
										   PDFSignatureInfo> {

	static final Logger tracer = Logger.getLogger ( PDFSignatureServiceItext.class );
	...
	public DigestSignature < SignCategory, 
		...
		try {
	        ...
	        PdfPKCS7 sgn = new PdfPKCS7(null, chain, null,hashAlgorithm, BouncyCastleProvider.PROVIDER_NAME, false);
	        InputStream data = sap.getRangeStream();
	        byte hash[] = DigestUtils.getDigest(hashAlgorithm).digest(IOUtils.toByteArray(data));//DigestAlgorithms.digest(data, externalDigest.getMessageDigest(hashAlgorithm));
	        //byte[] authenticatedAttributeBytes = sgn.getAuthenticatedAttributeBytes(hash, now, null, null, sigtype);
	        byte[] authenticatedAttributeBytes = sgn.getAuthenticatedAttributeBytes(hash, now, null);
	        // ---------------------------------------
...
//	        byte[] encodedSig = sgn.getEncodedPKCS7(hash, _getSignDate(doc.getSignDate()), tsaClient, ocsp, crlBytes, sigtype);
	        ...
	        byte[] encodedSig = sgn.getEncodedPKCS7(hash, cal, tsaClient, null);
...
//			byte[] digitalSignature;
	        ...
	        for (Object signNameO : acroFields.getSignatureNames() ) {
	        	try {
	        		String signName = String.valueOf(signNameO);
...
//					digitalSignature = pdfPkCs7.getEncodedPKCS7();
					...
					boolean verified = pdfPkCs7.verify(); //TODO make update
							//&& StringUtils.isBlank (CertificateVerification.verifyCertificate((X509Certificate)pkc[0], new ArrayList<CRL>(), cal) );
					if ( verified ) { 
