...
//			signatureDisposition = SignDisposition.CMS.EMBEDDED;
		...
		try {
			certHolderStore = signedData.getCertificates(null,"BC");
		} catch (NoSuchProviderException | NoSuchStoreException | CMSException e1) {
...
//        	}
			...
			//Selector selector = new X509CertificateHolderSelector(signer.getSID().getSubjectKeyIdentifier());          
			//certHolder = (X509CertificateHolder)certHolderStore.getMatches(signerId.).iterator().next();

			X509CertSelector signerConstraints = signer.getSID();
			certHolder = (X509CertificateHolder) certHolderStore.getMatches((Selector) signerConstraints).iterator().next();
...
//            	if ( signer.verify(certHolderConverter.getCertificate(certHolder), "BC") ) {
            	if ( signer.verify(new JcaX509CertificateConverter().setProvider( "BC" ).getCertificate(verifier.getAssociatedCertificate()),"BC") ) {
