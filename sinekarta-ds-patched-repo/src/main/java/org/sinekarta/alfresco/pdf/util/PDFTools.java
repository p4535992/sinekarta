...
public class PDFTools {
	...
	public static void signAndMark(DigitalSignatureDocument doc,
		...
		try {
			...
			signatureApparence.setCrypto(null, certList[0], null, null);
			...
			DEREncodableVector digestAlgorithm = new DEREncodableVector();
			...
			digestAlgorithms.add(new DERSequence(digestAlgorithm.get(0)));
			digestAlgorithms.add(new DERSequence(digestAlgorithm.get(1)));
			...
			body.add(digestAlgorithms.get(0));
...
//			body.add(contentinfoEncoding);
	...
	public static void calculateFingerPrint(DigitalSignatureDocument doc,
		...
		try {
			...
			sap.setCrypto(null, (X509Certificate) chain[0], null, null);
	...
	public static String getPrintableTimestampToken(InputStream is) {
		...
		try {
			reader = new PdfReader(is);
			AcroFields af = reader.getAcroFields();
			ArrayList<String> list = af.getSignatureNames();
