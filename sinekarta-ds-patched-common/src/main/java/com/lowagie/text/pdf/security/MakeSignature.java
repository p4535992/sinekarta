...
import com.lowagie.text.pdf.io.RASInputStream;
import com.lowagie.text.pdf.io.RandomAccessSource;
import com.lowagie.text.pdf.io.RandomAccessSourceFactory;
import com.lowagie.text.pdf.io.StreamUtil;
...
public class MakeSignature {
	...
	/** The Logger instance. */
    private static final org.slf4j.Logger LOGGER = org.slf4j.LoggerFactory.getLogger(MakeSignature.class);
    ...
    public static void signDetached(PdfSignatureAppearance sap, ExternalDigest externalDigest, ExternalSignature externalSignature, Certificate[] chain, Collection<CrlClient> crlList, OcspClient ocspClient,
        ...
        //	sap.addDeveloperExtension(PdfDeveloperExtension.ADOBE_1_7_EXTENSIONLEVEL3);
        //}
    ...
    public static void signDeferred(PdfReader reader, String fieldName, OutputStream outs, ExternalSignatureContainer externalSignatureContainer) throws DocumentException, IOException, GeneralSecurityException {
        ...
        for (int i = 0; i < b.size(); i++) {
            PdfNumber pdfNumber = b.getAsNumber(i);
            ll.add(Long.valueOf(pdfNumber.intValue()));
        }
        Long[] gaps = ll.toArray(new Long[ll.size()]);

        if (b.size() != 4 || gaps[0] != 0)
            throw new DocumentException("Single exclusion space supported");
        RandomAccessFileOrArray readerSource = reader.getSafeFile();//.createSourceView();
        InputStream rg = new RASInputStream(new RandomAccessSourceFactory().createRanged((RandomAccessSource)readerSource, gaps));
        ...
        StreamUtil.CopyBytes((RandomAccessSource)readerSource, 0, gaps[1] + 1, outs);
        ...
        bb.writeTo(outs);
        StreamUtil.CopyBytes((RandomAccessSource)readerSource, gaps[2] - 1, gaps[3] + 1, outs);
    }
}
